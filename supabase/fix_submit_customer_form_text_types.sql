-- submit_customer_form RPC의 p_business_age, p_monthly_revenue 파라미터 타입을 text로 변경
-- 컬럼 타입을 text로 바꾼 fix_business_age_monthly_revenue_text.sql 실행 후 적용
-- 실행: supabase db query --linked --file supabase/fix_submit_customer_form_text_types.sql

-- 기존 함수 시그니처 제거 (파라미터 타입 변경 시 DROP 후 재생성 필요)
DROP FUNCTION IF EXISTS public.submit_customer_form(
  uuid, text, text, text, text, text, text, int, numeric, boolean, numeric, text, text[], date, text
);

CREATE OR REPLACE FUNCTION public.submit_customer_form(
  p_workspace_id    uuid,
  p_company         text,
  p_phone           text      DEFAULT NULL,
  p_lead_source     text      DEFAULT NULL,
  p_business_type   text      DEFAULT NULL,
  p_industry        text      DEFAULT NULL,
  p_region          text      DEFAULT NULL,
  p_business_age    text      DEFAULT NULL,
  p_monthly_revenue text      DEFAULT NULL,
  p_tax_delinquent  boolean   DEFAULT false,
  p_required_funds  numeric   DEFAULT NULL,
  p_consultation_memo text    DEFAULT NULL,
  p_tags            text[]    DEFAULT NULL,
  p_received_date   date      DEFAULT NULL,
  p_business_reg_no text      DEFAULT NULL
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_customer_id uuid;
  v_allowed_ws  uuid := 'a1650b9e-31da-43e6-9179-17390d06f58c';
BEGIN
  IF p_workspace_id IS DISTINCT FROM v_allowed_ws THEN
    RAISE EXCEPTION '허용되지 않은 워크스페이스입니다.';
  END IF;

  IF p_company IS NULL OR trim(p_company) = '' THEN
    RAISE EXCEPTION '업체명은 필수 입력값입니다.';
  END IF;

  INSERT INTO public.customers (
    workspace_id,
    company,
    phone,
    lead_source,
    business_type,
    industry,
    region,
    business_age,
    monthly_revenue,
    tax_delinquent,
    required_funds,
    consultation_memo,
    tags,
    received_date,
    status,
    score,
    pool,
    business_reg_no
  ) VALUES (
    p_workspace_id,
    trim(p_company),
    p_phone,
    p_lead_source,
    p_business_type,
    p_industry,
    p_region,
    p_business_age,
    p_monthly_revenue,
    COALESCE(p_tax_delinquent, false),
    p_required_funds,
    p_consultation_memo,
    p_tags,
    COALESCE(p_received_date, CURRENT_DATE),
    '신규',
    0,
    false,
    p_business_reg_no
  )
  RETURNING id INTO v_customer_id;

  RETURN v_customer_id;
END;
$$;

GRANT EXECUTE ON FUNCTION public.submit_customer_form(
  uuid, text, text, text, text, text, text, text, text, boolean, numeric, text, text[], date, text
) TO anon, authenticated;
