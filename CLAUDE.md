...
#### 모든 작업 시작 전 필수 절차
1. 문제/요청을 있는 그대로 받아들이지 말 것 — 근본 원인과 숨겨진 연관 이슈까지 스스로 추론
2. 관련 파일 전체를 grep/read로 먼저 파악한 뒤 작업 착수
3. 단일 에이전트로 해결 가능해도 검증 역할로 두 번째 에이전트 투입

#### 작업 단계 구조 (모든 요청에 적용)
- Phase 1 — 탐색: 코드베이스 전체 스캔, 원인 후보 목록화
- Phase 2 — 진단: 각 후보 검증, 근본 원인 1개로 확정
- Phase 3 — 수정: 원인 직접 수정 + 동일 패턴 전체 보강
- Phase 4 — 검증: 수정 후 관련 케이스 전체 재확인
- Phase 5 — 보고: 원인/수정내용/잔여위험 한 줄씩 요약

#### 절대 하지 말 것
- 원인 불확실한 상태에서 코드 수정 착수
- 에러 메시지만 보고 표면적 수정 (500 → try-catch 감싸기만 하는 식)
- 수정 후 검증 없이 완료 보고
- 하나의 에이전트에게 탐색~수정~검증 전부 위임
